...
`Left` как подросток, игнорирует наши просьбы обработать значение через `map`. `Right` будет работать точно так же как `Container` (он же Identity). Мощь заключается в том, что у нас есть возможность встроить сообщение об ошибке внутри `Left`.

Предположим, что у нас есть функция, которая может завершиться с ошибкой. Например, вычисление возраста по дате рождения. Мы могли бы использовать `Nothing`/`Maybe(null)`, чтобы сообщить об  ошибке и перенаправить нашу программу, однако, это мало о чем нам скажет. Возможно, мы бы хотели  знать почему произошла ошибка. Давайте реализуем это используя `Either`:
...
Теперь, Прямо как `Nothing`/`Maybe(null)`, Мы упрощаем приложения возвращая `Left`. Разница в том что, теперь мы имеем понимание о том, почему наша программа сорвалась. Стоит заметить, что мы возвращаем `Either(String, Number)`, который содержит `String`, как его левое значени и  `Number`, как правое `Right`. Эта сигнатуры функции немного неформальная поскольку мы не нашли времени, чтобы определить родителский класс `Either`, одако, мы можем многое подчерпнуть из этого типа. Он говорит нам, что мы получаем сообщение об ошибке или возраст.
...
Если `birthDate` валидное значние, то программа выведет на экран таинтсвенное значение(ваш возраст), чтобы мы смогли его созерцать. Иначе, он покажет вам `Left` c сообщением об ошибке, ясное как божий день, хотя и по прежнему внутри вашего контейнера. Таким образом, это показывает нам ошибку, но в спокойной и вежливой форме, в отличие от ребенка, который теряет контроль, когда что-то идет не так.

В этом примере, мы логически  выбираем нашу ветку выполнения в завимости от того, валидные ли данные о дне рождения или нет, мы движемся линейно справа налево одним движением, вместо того, чтобы подниматься через условные операторы.  Как правило, мы обычно выносим  `console.log` из таких функции как `zoltar` и обрабатываем её с помощью `map` в момент вызова, но в даном случае полезно видеть, как отличается ветка `Right`. Мы используем `_` в правой части сигнатуры функции для того, чтобы указать что это значение должно быть проигнорировано (В некоторых браузерах вам нужно будет использовать `console.log.bind(console)`, чтобы вы смогли использовать его как first class object).

Я бы хотел остановится на секунду, чтобы подчеркнуть то, что вы, возможно, пропустили: `fortune`, несмотря на его совместное использование с `Either` в этом примере, не знает ничего о какой-либо связи с функтором. Тоже самое было и в случае с `finishTransaction` в предыдущем пример. Во время вызова, функция может быть обернута в `map`, который траснформирует её  из той, которая не связана с функтором в ту, которая в функторе. Мы называем этот процесс *lifting/подъем*. Часто функции проще работать с обычными типами, а не с контейнерами, а затем подниматься в нужный контейнер, если это необходимо. Это придает простоту и делает функции более многоразовыми, теми которые могут быть изменены для работы с любым функтором по требованию.

`Either` отлично подходит как для обычных ошибок, таких как валидационные проверки, так и для более серьезных вещей, например недостающие файлы или проблем с сокетами. Попытайтесь заменить в некоторых из примеров `Maybe` на `Either`, чтобы получить лучший отклик.

На данный момент, Я не могу не думать о том, что я сделал `Either` медвежью услугу, когда представил его как просто контейнер для сообщений об ошибках. `Either` представляет логическое ИЛИ (то есть `||`). Он также воплощает идею `Coproduct` из теории категорий (мы не будем рассматривать её в этой книге, но про неё стоит прочитать, потому что она имеет много свойств для изучения). Кроме того, это стандартный тип суммы (или несвязанный союз множеств), поскольку общее количество всех возможных значений, которые оно содержит, является суммой двух его типов (я знаю, что вы читате это, но не понимаете, вот очень хорошая статья об этой проблеме [great article](https://www.fpcomplete.com/school/to-infinity-and-beyond/pick-of-the-week/sum-types)).  Есть много вещей, которые `Either` может сделать, но как функтор, он используется для обработки ошибок.
...
Здесь `ioWindow` это фактический `IO`, который мы можем сразу обрабатывать c помощью `map`, в то время как `$` - это функция, которая возвращает `IO` после того, как она была вызвана. Я написал *концептуальные* возвращаемые значения, чтобы лучше показать `IO`, тем не менее, в действительности, оно всегда будет `{ $value: [Function] }`. Когда мы c помощью `map` обрабатываем наш `IO`, мы отправляем в конец композиции ту функцию, которая, в свою очередь, станет новым `$value`. Наши замапленные функции не запускаются. Они  просто занимают свое место в конце цепочки вычислений, которую мы строим, функция за функцией, подобно тому, как мы осторожно расставляем домино стараясь ничего не задеть. Результат напоминает паттерн проектирования "очередь" или "команда" из книги «Банды четырех»
...
Есть кое-что что меня беспокоит в течение длительного времени, и мы должны исправить это немедленно: `$value` в `IO` это на самом деле не значение, которое он содержит и не его частное свойство. Это чека на гранате,и вытащит эту чеку тот, кто вызовет это свойство. Давайте переменуем это свойство в  `unsafePerformIO`, чтобы напомнить нашим пользователям о его непостоянстве.
...
Функции, которые я называю `reject` и `result`, это наши функции обратного вызова `error` и `success` соотвественно. Как вы можете видеть, Мы просто обрабатываем `Task` c помощью `map`, работая над будущим значением, как если бы оно  было прямо здесь под рукой. К настоящему времени, `map` уже должен был порядком приесться.
...
Потратьте минутку, чтобы оценить на сколько линейный поток управления здесь. Мы можем просто читать снизу вверх, справа налево, несмотря на то, что во время выполнения программа будет прыгать по коду. Это делает чтение и понимание того, что происходит внутри приложения гораздо более простым, чем переход между различными блоками обработки обратных вызовов и блоками обработки ошибок.

Боже мой, посмотрите на этот `Task`! Он также включает в себя `Either`! И Он должен был это сделать потому, что наш предыдущий поток управления не применяется в асинхронном мире, где мы можем иметь дело с потенциальными ошибками. Это все просто замечательно, так как это обеспечивает нам достаточную и чистую обработку ошибок прямо из коробки.
...
Используйте функции `add` и `map` для создания функции, котороая увеличивает значение внутри функтора.
...
Используйте `safeProp` и `head` для того, чтобы найти первую букву имени пользователя.
...
Напишите функцию `validateName`, которая проверяет длину имени пользователя, и если длина меньше чем 3 символа, то возвращает ошибку. Затем используейте `either`, `showWelcome` и `save`, чтобы написать функцию `register`, чтобы зарегистрировать и поприветствовать пользователя, если он прошел валидацию. 
